export type SystemLanguage = 'ru' | 'az'

export const SYSTEM_LANGUAGE_STORAGE_KEY = 'freedom-platform:system-language'

export const languageLabels: Record<SystemLanguage, string> = {
  ru: 'Русский',
  az: 'Azərbaycan dili',
}

export const supportedLanguages: SystemLanguage[] = ['ru', 'az']

export const azTranslations: Record<string, string> = {
  '(обязательно)': '(vacibdir)',
  ' · через полночь': ' · gecə yarısından sonra',
  '/the-liga/admin и /the-liga/employee': '/the-liga/admin və /the-liga/employee',
  'Email недоступен': 'Email əlçatan deyil',
  'Email существующего пользователя': 'Mövcud istifadəçinin emaili',
  'P&L по начислению: выручка, COGS по snapshots, расходы и чистая прибыль.':
    'Hesablama üzrə P&L: gəlir, snapshot-lar üzrə COGS, xərclər və xalis mənfəət.',
  'PIN для рабочего места не настроен.': 'İş yeri üçün PIN qurulmayıb.',
  'PIN должен состоять из 4 цифр.': 'PIN 4 rəqəmdən ibarət olmalıdır.',
  'PIN ещё не задан.': 'PIN hələ təyin edilməyib.',
  'PIN задан и может использоваться для блокировки рабочего экрана.':
    'PIN təyin edilib və iş ekranını kilidləmək üçün istifadə oluna bilər.',
  'PIN начнёт работать после одобрения администратора.':
    'PIN administrator təsdiqindən sonra işləyəcək.',
  'PIN ожидает одобрения': 'PIN təsdiq gözləyir',
  'PIN ожидает одобрения администратора.': 'PIN administrator təsdiqini gözləyir.',
  'PNG, JPG или WebP. Нажмите, чтобы выбрать фотографию.':
    'PNG, JPG və ya WebP. Şəkil seçmək üçün klikləyin.',
  'RLS и роли Supabase': 'RLS və Supabase rolları',
  'Readiness пока недоступен': 'Hazırlıq yoxlaması hələ əlçatan deyil',
  'Setup организации': 'Təşkilatın qurulması',
  'Slug организаций': 'Təşkilat slug-ları',
  'Slug: lowercase, цифры и дефисы.': 'Slug: kiçik hərflər, rəqəmlər və defislər.',
  'Timed места': 'Vaxtla işləyən yerlər',
  'UUID администратора': 'Administrator UUID-si',
  'UUID существующего пользователя для роли администратора':
    'Administrator rolu üçün mövcud istifadəçinin UUID-si',
  'useAuth должен использоваться внутри AuthProvider.':
    'useAuth AuthProvider daxilində istifadə olunmalıdır.',
  'Автоматические доходы из оплаченных заказов и ручной доход организации.':
    'Ödənilmiş sifarişlərdən avtomatik gəlirlər və təşkilatın əl ilə daxil edilən gəliri.',
  Администратор: 'Administrator',
  'Администратор уже имеет доступ к рабочему месту. Откройте раздел "Рабочее место" в левом меню: там он сможет открыть смену, создавать заказы и принимать оплату.':
    'Administratorun artıq iş yerinə girişi var. Sol menyudan "İş yeri" bölməsini açın: orada növbə aça, sifariş yarada və ödəniş qəbul edə bilər.',
  'Администратор уже может работать через раздел "Рабочее место".':
    'Administrator artıq "İş yeri" bölməsi vasitəsilə işləyə bilər.',
  'Администратор организации': 'Təşkilat administratoru',
  'Аккаунт отключен. Обратитесь к владельцу Freedom Platform.':
    'Hesab deaktiv edilib. Freedom Platform sahibinə müraciət edin.',
  Активен: 'Aktivdir',
  Активна: 'Aktivdir',
  'Активная организация не выбрана или доступ был приостановлен.':
    'Aktiv təşkilat seçilməyib və ya giriş dayandırılıb.',
  Активно: 'Aktivdir',
  Активные: 'Aktivlər',
  'Активные сессии': 'Aktiv sessiyalar',
  Архив: 'Arxiv',
  Архивировать: 'Arxivləşdir',
  Баланс: 'Balans',
  Банк: 'Bank',
  'Без reference': 'Referencesiz',
  'Без фиксированного времени': 'Sabit vaxt yoxdur',
  'Без имени': 'Adsız',
  'Без комментария': 'Şərhsiz',
  'Без места': 'Yersiz',
  'Без поставщика': 'Təchizatçısız',
  'Без профиля': 'Profilsiz',
  'Без таймера': 'Taymersiz',
  'Без шаблона': 'Şablonsuz',
  Безопасность: 'Təhlükəsizlik',
  Бильярд: 'Bilyard',
  'Браузер не поддерживает обработку изображений.':
    'Brauzer şəkil emalını dəstəkləmir.',
  'Браузер не смог создать WebP изображение.':
    'Brauzer WebP şəkli yarada bilmədi.',
  'В наличии': 'Mövcuddur',
  'Валовая прибыль': 'Ümumi mənfəət',
  Валюта: 'Valyuta',
  'Валюта отчёта': 'Hesabat valyutası',
  'Валюта по умолчанию': 'Standart valyuta',
  Войти: 'Daxil ol',
  'Будут удалены заказы, платежи, смены, операционные дни, доходы от заказов и складские списания по заказам. Товары, услуги, комбо, складские документы и расходы останутся.':
    'Sifarişlər, ödənişlər, növbələr, əməliyyat günləri, sifariş gəlirləri və sifariş üzrə anbar silinmələri silinəcək. Məhsullar, xidmətlər, kombo, anbar sənədləri və xərclər qalacaq.',
  'Быстрое изменение количества сотрудником.':
    'Əməkdaş tərəfindən sürətli miqdar dəyişikliyi.',
  'Ваш аккаунт создан, но доступ к организации еще не настроен.':
    'Hesabınız yaradılıb, amma təşkilata giriş hələ qurulmayıb.',
  'Ваш аккаунт создан, но доступ к организации еще не настроен. Обратитесь к владельцу Freedom Platform.':
    'Hesabınız yaradılıb, amma təşkilata giriş hələ qurulmayıb. Freedom Platform sahibinə müraciət edin.',
  'Введите 4 цифры.': '4 rəqəm daxil edin.',
  'Введите корректный email.': 'Düzgün email daxil edin.',
  'Введите логин или email.': 'Login və ya email daxil edin.',
  'Введите название организации.': 'Təşkilat adını daxil edin.',
  'Введите название.': 'Ad daxil edin.',
  'Введите пароль': 'Şifrəni daxil edin',
  'Введите RESET_TEST_ORDERS для окончательной очистки.':
    'Son təmizləmə üçün RESET_TEST_ORDERS daxil edin.',
  Включено: 'Aktivdir',
  Включить: 'Aktiv et',
  'В админку': 'Admin panelə',
  'Владелец платформы': 'Platforma sahibi',
  Вместимость: 'Tutum',
  'Вместимость должна быть больше 0.': 'Tutum 0-dan böyük olmalıdır.',
  'Возврат приход': 'Geri qaytarma mədaxil',
  'Возврат расход': 'Geri qaytarma məxaric',
  Восстановить: 'Bərpa et',
  'Восстановить доступ': 'Girişi bərpa et',
  Все: 'Hamısı',
  'Все статусы': 'Bütün statuslar',
  'Все категории': 'Bütün kateqoriyalar',
  'Все типы': 'Bütün növlər',
  'Выберите организацию или платежи в разделе финансов.':
    'Maliyyə bölməsində təşkilat və ya ödənişləri seçin.',
  'Выберите товар.': 'Məhsul seçin.',
  'Выберите файл изображения.': 'Şəkil faylı seçin.',
  'Выбрать фотографию': 'Şəkil seç',
  Выключен: 'Deaktivdir',
  Выключена: 'Deaktivdir',
  Выключено: 'Deaktivdir',
  'Выручка сегодня': 'Bugünkü gəlir',
  Высота: 'Hündürlük',
  'Высота должна быть больше 0.': 'Hündürlük 0-dan böyük olmalıdır.',
  'Время идёт. Завершите сессию, затем укажите сумму, которую клиент оставил.':
    'Vaxt gedir. Sessiyanı tamamlayın, sonra müştərinin qoyduğu məbləği göstərin.',
  'Глобальный контроль финансов организаций, периодов и задолженности по доле платформы.':
    'Təşkilat maliyyəsi, dövrlər və platforma payı borcları üzrə ümumi nəzarət.',
  Готово: 'Hazırdır',
  'Группы для мест, товаров и услуг.': 'Yerlər, məhsullar və xidmətlər üçün qruplar.',
  Да: 'Bəli',
  'Данные доступны в соседних разделах финансового блока.':
    'Məlumatlar maliyyə blokunun qonşu bölmələrində mövcuddur.',
  Дата: 'Tarix',
  'Дата начисления': 'Hesablama tarixi',
  'Дата оплаты': 'Ödəniş tarixi',
  'Движение денег по датам оплаты за текущий месяц.':
    'Cari ay üçün ödəniş tarixlərinə görə pul hərəkəti.',
  'Действует с': 'Başlama tarixi',
  'День закрытия месяца': 'Ayın bağlanma günü',
  'День открытия': 'Açılış günü',
  'Первая смена': 'Birinci növbə',
  'Вторая смена': 'İkinci növbə',
  'Детальный финансовый период и расчёт доли Freedom Platform.':
    'Detallı maliyyə dövrü və Freedom Platform payının hesablanması.',
  'Длина PIN сотрудника': 'Əməkdaş PIN uzunluğu',
  'Для drag-and-drop нужно применить миграцию workspace layout в Supabase.':
    'Drag-and-drop üçün Supabase-də workspace layout miqrasiyasını tətbiq edin.',
  'Для аккаунта нет активного доступа к организации.':
    'Hesabın təşkilata aktiv girişi yoxdur.',
  'Для перемещения и изменения размера нужно применить миграцию workspace layout в Supabase.':
    'Daşıma və ölçü dəyişməsi üçün Supabase-də workspace layout miqrasiyasını tətbiq edin.',
  'Для почасовой услуги нужны тариф, минимум и шаг.':
    'Saatlıq xidmət üçün tarif, minimum və addım lazımdır.',
  'Для таймера нужны тариф, минимум и шаг.':
    'Taymer üçün tarif, minimum və addım lazımdır.',
  'Добавить места и рабочую схему': 'Yerləri və iş sxemini əlavə et',
  'Добавить сотрудника': 'Əməkdaş əlavə et',
  'Добавить товары': 'Məhsullar əlavə et',
  'Добавить услуги': 'Xidmətlər əlavə et',
  'Добавьте компонент.': 'Komponent əlavə edin.',
  'Добавьте позиции или переведите заказ к оплате.':
    'Mövqelər əlavə edin və ya sifarişi ödənişə keçirin.',
  'Добавьте существующего Supabase Auth пользователя как employee этой организации.':
    'Mövcud Supabase Auth istifadəçisini bu təşkilatın əməkdaşı kimi əlavə edin.',
  'Добавьте хотя бы одну позицию.': 'Ən azı bir mövqe əlavə edin.',
  'Документы склада': 'Anbar sənədləri',
  Должность: 'Vəzifə',
  'Должность не указана': 'Vəzifə göstərilməyib',
  'Доля платформы': 'Platforma payı',
  'Доля платформы, %': 'Platforma payı, %',
  'Домен и ссылки': 'Domen və linklər',
  'Доступ найден, но отключен. Можно восстановить.':
    'Giriş tapıldı, amma deaktivdir. Bərpa etmək olar.',
  'Доступа к этой организации пока нет.': 'Bu təşkilata giriş hələ yoxdur.',
  'Дополнительная сумма от клиента': 'Müştəridən əlavə məbləğ',
  Доход: 'Gəlir',
  Доходы: 'Gəlirlər',
  'Доходы, расходы, периоды и доля платформы.':
    'Gəlirlər, xərclər, dövrlər və platforma payı.',
  Другое: 'Digər',
  'Единица учета': 'Uçot vahidi',
  'Есть хотя бы одно место с почасовой тарификацией.':
    'Saatlıq tariflə işləyən ən azı bir yer var.',
  'Если чаевых нет, оставьте 0 или пусто.':
    'Çaypulu yoxdursa, 0 yazın və ya boş saxlayın.',
  Заблокировать: 'Kilidlə',
  'Заблокировать рабочий экран': 'İş ekranını kilidlə',
  'Заблокировать экран': 'Ekranı kilidlə',
  Завершение: 'Tamamlama',
  Завершить: 'Tamamla',
  'Завершить заказ?': 'Sifarişi tamamlamaq?',
  'Загрузите фото места.': 'Yerin şəklini yükləyin.',
  'Загрузите фото товара.': 'Məhsul şəklini yükləyin.',
  'Загрузите фото услуги.': 'Xidmət şəklini yükləyin.',
  'Загрузить логотип': 'Loqo yüklə',
  'Меню': 'Menyu',
  Мониторинг: 'Monitorinq',
  'Мониторинг мест': 'Yerlərin monitorinqi',
  'Мобильный мониторинг': 'Mobil monitorinq',
  'Места онлайн': 'Yerlər onlayn',
  'Смотреть все': 'Hamısına bax',
  'Смотреть места': 'Yerlərə bax',
  'Просмотр смены, мест, заказов и товаров без рабочих действий.':
    'İş əməliyyatları olmadan növbə, yerlər, sifarişlər və məhsullara baxış.',
  'Загрузка меню…': 'Menyu yüklənir…',
  'Загрузка данных...': 'Məlumatlar yüklənir...',
  'Детали смены': 'Növbə detalları',
  'Наличные продажи': 'Nağd satışlar',
  'Переводы на карту': 'Karta köçürmələr',
  'Фактическая касса': 'Faktiki kassa',
  'Начальная касса': 'Başlanğıc kassası',
  Расхождение: 'Fərq',
  Ответственный: 'Məsul şəxs',
  'Роль не указана': 'Rol göstərilməyib',
  'Закрыть админом': 'Administrator kimi bağla',
  'Номер заказа': 'Sifariş nömrəsi',
  'Заказов нет': 'Sifariş yoxdur',
  'Сессий нет': 'Sessiya yoxdur',
  'Передача смены': 'Növbə təhvili',
  'Передач нет': 'Təhvil yoxdur',
  Открытые: 'Açıq',
  Завершено: 'Tamamlanıb',
  Возврат: 'Geri qaytarma',
  Завершена: 'Tamamlanıb',
  Отменена: 'Ləğv edilib',
  Факт: 'Fakt',
  мин: 'dəq',
  'Ошибка загрузки меню': 'Menyu yüklənərkən səhv',
  // item types (used by employee menu page)
  product: 'məhsul',
  service: 'xidmət',
  combo: 'kombo',
  // 'Комбо' translation removed to avoid duplicate key; use item type key `combo` above
  'Показать состав': 'Tərkibi göstər',
  'Скрыть состав': 'Tərkibi gizlət',
  'Загрузка организации': 'Təşkilat yüklənir',
  'Загрузка статуса PIN...': 'PIN statusu yüklənir...',
  'Задана ставка Freedom Platform.': 'Freedom Platform dərəcəsi təyin edilib.',
  'Задать PIN': 'PIN təyin et',
  'Задержка закрытия, мин': 'Bağlanma gecikməsi, dəq',
  Заказ: 'Sifariş',
  'Заказ готов к оплате.': 'Sifariş ödənişə hazırdır.',
  'Заказ не открыт': 'Sifariş açıq deyil',
  Заказы: 'Sifarişlər',
  Закрывается: 'Bağlanır',
  Закрыт: 'Bağlıdır',
  Закрыта: 'Bağlıdır',
  'Закрыта админом': 'Administrator tərəfindən bağlanıb',
  'Закрытие финансовых периодов и отправка на проверку платформе.':
    'Maliyyə dövrlərinin bağlanması və platforma yoxlamasına göndərilməsi.',
  Закрыть: 'Bağla',
  Закупка: 'Satınalma',
  Закупки: 'Satınalmalar',
  'Закупки из складских документов. Они влияют на cash flow, но не попадают в P&L как COGS.':
    'Anbar sənədlərindən satınalmalar. Cash flow-a təsir edir, amma P&L-də COGS kimi düşmür.',
  'Закупочная цена': 'Alış qiyməti',
  Занято: 'Məşğuldur',
  'Запросы на удаление и корректировки.': 'Silinmə və düzəliş sorğuları.',
  'Заявка на PIN отправлена администратору.': 'PIN sorğusu administratora göndərildi.',
  'Заявка на смену PIN': 'PIN dəyişmə sorğusu',
  'Значения по умолчанию для новых организаций и финансовых периодов.':
    'Yeni təşkilatlar və maliyyə dövrləri üçün standart dəyərlər.',
  Зона: 'Zona',
  'Измените поиск или фильтр, чтобы увидеть сотрудников.':
    'Əməkdaşları görmək üçün axtarışı və ya filtri dəyişin.',
  'Измените фильтр или поиск.': 'Filtri və ya axtarışı dəyişin.',
  'Изменение количества': 'Miqdar dəyişikliyi',
  'Изменение метода оплаты': 'Ödəniş metodunun dəyişdirilməsi',
  'Изменения заказа': 'Sifariş dəyişiklikləri',
  'Изменений по фильтру нет': 'Filtrə uyğun dəyişiklik yoxdur',
  Изображение: 'Şəkil',
  Имя: 'Ad',
  'Имя пользователя': 'İstifadəçi adı',
  'Имя сотрудника': 'Əməkdaş adı',
  'Имя клиента': 'Müştərinin adı',
  'Имя, email, должность или телефон': 'Ad, email, vəzifə və ya telefon',
  Исправления: 'Düzəlişlər',
  Истекло: 'Vaxtı keçib',
  'История пуста': 'Tarixçə boşdur',
  'История смен и закрытия кассы.': 'Növbələr və kassanın bağlanması tarixçəsi.',
  'История товара': 'Məhsul tarixçəsi',
  'Итого с чаевыми': 'Çaypulu ilə cəmi',
  'К оплате': 'Ödənişə',
  Кабинет: 'Kabinet',
  Касса: 'Kassa',
  Каталог: 'Kataloq',
  'Каталог продаж с ценами и базовым остатком.':
    'Qiymətlər və baza qalıqları ilə satış kataloqu.',
  Категории: 'Kateqoriyalar',
  'Категорий пока нет': 'Kateqoriyalar hələ yoxdur',
  'Код валюты должен содержать 3 символа.': 'Valyuta kodu 3 simvoldan ibarət olmalıdır.',
  'Кол-во': 'Say',
  'Количество должно быть больше 0.': 'Miqdar 0-dan böyük olmalıdır.',
  'Колонка X': 'X sütunu',
  'Колонка от 1 до 12.': 'Sütun 1-dən 12-yə qədər olmalıdır.',
  'Когда администратор начнёт открывать разделы или выполнять операции, события появятся здесь.':
    'Administrator bölmələri açmağa və ya əməliyyatlar etməyə başlayanda hadisələr burada görünəcək.',
  Комбо: 'Kombo',
  'Комбо пока нет': 'Kombo hələ yoxdur',
  Комментарий: 'Şərh',
  'Комментарий к заказу': 'Sifariş şərhi',
  'Комментарий к одобрению': 'Təsdiq şərhi',
  'Комментарий к отказу от оплаты': 'Ödənişdən imtina şərhi',
  Компакт: 'Kompakt',
  Компонент: 'Komponent',
  Конец: 'Son',
  Копировать: 'Kopyala',
  'Корректировка +': 'Düzəliş +',
  'Корректировка -': 'Düzəliş -',
  'Крупный расход от': 'Böyük xərc limiti',
  'Лимиты организаций': 'Təşkilat limitləri',
  'Логин или email': 'Login və ya email',
  'Логотип организации': 'Təşkilat loqosu',
  'Максимум организаций': 'Maksimum təşkilat sayı',
  Места: 'Yerlər',
  Место: 'Yer',
  'Минимальный остаток': 'Minimum qalıq',
  'Минимум должен быть больше 0.': 'Minimum 0-dan böyük olmalıdır.',
  'Минимум минут': 'Minimum dəqiqə',
  'Минимум не может быть отрицательным.': 'Minimum mənfi ola bilməz.',
  'Можно добавить позиции или завершить пустой заказ без оплаты.':
    'Mövqelər əlavə edə və ya boş sifarişi ödənişsiz tamamlaya bilərsiniz.',
  'Можно указать 0, если сегодня денег не взяли.':
    'Bu gün pul alınmayıbsa, 0 göstərmək olar.',
  'Например: клиент оставил больше, оплата от друга, особые условия.':
    'Məsələn: müştəri artıq pul qoydu, ödəniş dostdan gəldi, xüsusi şərtlər.',
  'Например: Эльвин': 'Məsələn: Elvin',
  'На проверке': 'Yoxlamadadır',
  'На этом месте уже запущена активная сессия.': 'Bu yerdə artıq aktiv sessiya başladılıb.',
  Название: 'Ad',
  'Название или SKU': 'Ad və ya SKU',
  'Название или описание': 'Ad və ya təsvir',
  'Название или характеристика': 'Ad və ya xüsusiyyət',
  'Название типа': 'Növ adı',
  'Название, SKU, характеристика': 'Ad, SKU, xüsusiyyət',
  'Назначен активный администратор организации.': 'Aktiv təşkilat administratoru təyin edilib.',
  Наличные: 'Nağd',
  'Например: клиент отменил позицию': 'Məsələn: müştəri mövqeni ləğv etdi',
  'Например: клиент передумал': 'Məsələn: müştəri fikrini dəyişdi',
  'Настроены уведомления. Не блокирует запуск.':
    'Bildirişlər qurulub. Başlamağı bloklamır.',
  'Настроены шаблоны смен.': 'Növbə şablonları qurulub.',
  'Настроить шаблоны смен': 'Növbə şablonlarını qur',
  Настройки: 'Ayarlar',
  'Настройки организации': 'Təşkilat ayarları',
  'Настройки организации недоступны': 'Təşkilat ayarları əlçatan deyil',
  'Настройки финансов': 'Maliyyə ayarları',
  Начало: 'Başlanğıc',
  'Начальная наличность': 'Başlanğıc nağd pul',
  'Начальный остаток': 'Başlanğıc qalıq',
  'Начальный остаток при создании товара': 'Məhsul yaradılarkən başlanğıc qalıq',
  'Начисления и платежи доли Freedom Platform.':
    'Freedom Platform payı üzrə hesablamalar və ödənişlər.',
  'Не ведется': 'Aparılmır',
  'Не задан': 'Təyin edilməyib',
  'Не оплачен': 'Ödənilməyib',
  'Не удалось войти.': 'Daxil olmaq mümkün olmadı.',
  'Не удалось выполнить действие.': 'Əməliyyatı yerinə yetirmək mümkün olmadı.',
  'Не удалось добавить сотрудника.': 'Əməkdaşı əlavə etmək mümkün olmadı.',
  'Не удалось загрузить активные организации.': 'Aktiv təşkilatları yükləmək mümkün olmadı.',
  'Не удалось загрузить доступ к организациям.': 'Təşkilat girişlərini yükləmək mümkün olmadı.',
  'Не удалось загрузить организации платформы.': 'Platforma təşkilatlarını yükləmək mümkün olmadı.',
  'Не удалось загрузить профиль пользователя.': 'İstifadəçi profilini yükləmək mümkün olmadı.',
  'Не удалось назначить администратора.': 'Administrator təyin etmək mümkün olmadı.',
  'Не удалось найти пользователя.': 'İstifadəçini tapmaq mümkün olmadı.',
  'Не удалось обновить имя пользователя.': 'İstifadəçi adını yeniləmək mümkün olmadı.',
  'Не удалось одобрить PIN.': 'PIN-i təsdiqləmək mümkün olmadı.',
  'Не удалось отклонить заявку.': 'Sorğunu rədd etmək mümkün olmadı.',
  'Не удалось отправить заявку.': 'Sorğunu göndərmək mümkün olmadı.',
  'Не удалось проверить PIN.': 'PIN-i yoxlamaq mümkün olmadı.',
  'Не удалось проверить глобальную роль пользователя.':
    'İstifadəçinin qlobal rolunu yoxlamaq mümkün olmadı.',
  'Не удалось проверить текущую сессию.': 'Cari sessiyanı yoxlamaq mümkün olmadı.',
  'Не удалось прочитать изображение.': 'Şəkli oxumaq mümkün olmadı.',
  'Не удалось сохранить PIN.': 'PIN-i saxlamaq mümkün olmadı.',
  'Не удалось сохранить документ.': 'Sənədi saxlamaq mümkün olmadı.',
  'Не удалось сохранить категорию.': 'Kateqoriyanı saxlamaq mümkün olmadı.',
  'Не удалось сохранить комбо.': 'Kombonu saxlamaq mümkün olmadı.',
  'Не удалось сохранить место.': 'Yeri saxlamaq mümkün olmadı.',
  'Не удалось сохранить организацию.': 'Təşkilatı saxlamaq mümkün olmadı.',
  'Не удалось сохранить схему.': 'Sxemi saxlamaq mümkün olmadı.',
  'Не удалось сохранить товар.': 'Məhsulu saxlamaq mümkün olmadı.',
  'Не удалось удалить товар.': 'Məhsulu silmək mümkün olmadı.',
  'Не удалось очистить тестовые заказы.': 'Test sifarişlərini təmizləmək mümkün olmadı.',
  'Не удалось сохранить услугу.': 'Xidməti saxlamaq mümkün olmadı.',
  'Не удалось сохранить шаблон.': 'Şablonu saxlamaq mümkün olmadı.',
  'Не удалось обновить остаток.': 'Qalığı yeniləmək mümkün olmadı.',
  'Неверный PIN.': 'Yanlış PIN.',
  'Недостаточно остатка на складе.': 'Anbarda kifayət qədər qalıq yoxdur.',
  'Недостаточно прав для этого действия.': 'Bu əməliyyat üçün kifayət qədər hüquq yoxdur.',
  Недоступно: 'Əlçatan deyil',
  'Необязательно для организаций только с услугами.':
    'Yalnız xidmətlərlə işləyən təşkilatlar üçün vacib deyil.',
  'Необязательно, если организация работает только с товарами.':
    'Təşkilat yalnız məhsullarla işləyirsə, vacib deyil.',
  Нет: 'Xeyr',
  'Нет в наличии': 'Mövcud deyil',
  'Нет выбранного объекта': 'Seçilmiş obyekt yoxdur',
  'Низкий остаток': 'Aşağı qalıq',
  'Ничего не найдено': 'Heç nə tapılmadı',
  Осталось: 'Qalıb',
  'Открытых заказов нет': 'Açıq sifariş yoxdur',
  'Новое количество': 'Yeni miqdar',
  'Причина изменения количества': 'Miqdar dəyişikliyinin səbəbi',
  'Введите корректное количество.': 'Düzgün miqdar daxil edin.',
  'Введите новое количество, отличающееся от текущего остатка.':
    'Cari qalıqdan fərqli yeni miqdar daxil edin.',
  'Новый PIN должен состоять из 4 цифр.': 'Yeni PIN 4 rəqəmdən ibarət olmalıdır.',
  'Новый PIN начнёт работать только после одобрения администратора.':
    'Yeni PIN yalnız administrator təsdiqindən sonra işləyəcək.',
  'Номер, место, клиент': 'Nömrə, yer, müştəri',
  'Нужна проверка': 'Yoxlama lazımdır',
  'Нужны уточнения': 'Dəqiqləşdirmə lazımdır',
  Обзор: 'İcmal',
  'Обзор организации': 'Təşkilat icmalı',
  'Ограничения роста, хранения и активности tenant-организаций.':
    'Tenant təşkilatların böyümə, saxlama və aktivlik məhdudiyyətləri.',
  Одобрен: 'Təsdiqlənib',
  Одобрено: 'Təsdiqlənib',
  Ожидает: 'Gözləyir',
  'Ожидало подтверждения': 'Təsdiq gözləyirdi',
  'Ожидает оплату': 'Ödəniş gözləyir',
  'Ожидает оплаты': 'Ödəniş gözləyir',
  'Ожидает подтверждения': 'Təsdiq gözləyir',
  'Ожидают оплату': 'Ödəniş gözləyir',
  'Опер. расходы': 'Əməliyyat xərcləri',
  'Операций пока нет': 'Əməliyyat hələ yoxdur',
  'Операционная сводка по заказам, сменам, рабочим местам, складу и финансам.':
    'Sifarişlər, növbələr, iş yerləri, anbar və maliyyə üzrə əməliyyat xülasəsi.',
  'Операционные дни': 'Əməliyyat günləri',
  'Операционные расходы с учётом крупных расходов и approval workflow.':
    'Böyük xərclər və təsdiq axını nəzərə alınmaqla əməliyyat xərcləri.',
  'Операция не выполнена.': 'Əməliyyat yerinə yetirilmədi.',
  'Описание не заполнено.': 'Təsvir doldurulmayıb.',
  Оплачен: 'Ödənilib',
  Оплачено: 'Ödənilib',
  'Оплачено заказов': 'Ödənilmiş sifarişlər',
  Организации: 'Təşkilatlar',
  'Организации еще не созданы': 'Təşkilatlar hələ yaradılmayıb',
  'Организации пока не отправляли платежи на подтверждение.':
    'Təşkilatlar hələ təsdiq üçün ödəniş göndərməyib.',
  Организация: 'Təşkilat',
  'Организация или пользователь не определены.': 'Təşkilat və ya istifadəçi müəyyən edilməyib.',
  'Организация не выбрана.': 'Təşkilat seçilməyib.',
  'Организация отключена или архивирована. Рабочий доступ временно недоступен.':
    'Təşkilat deaktiv edilib və ya arxivləşdirilib. İş girişi müvəqqəti əlçatan deyil.',
  'Остатки, документы и движение товаров.': 'Qalıqlar, sənədlər və məhsul hərəkətləri.',
  'Остаток не может быть отрицательным.': 'Qalıq mənfi ola bilməz.',
  'Отказ от оплаты': 'Ödənişdən imtina',
  Отказы: 'İmtinalar',
  'Отказы от оплаты': 'Ödənişdən imtinalar',
  Отклонен: 'Rədd edilib',
  Отклонено: 'Rədd edilib',
  Отклонён: 'Rədd edilib',
  Отключен: 'Deaktivdir',
  Отключенные: 'Deaktivlər',
  Отключить: 'Deaktiv et',
  'Отключить доступ': 'Girişi deaktiv et',
  'Отключить доступ пользователя к организации?': 'İstifadəçinin təşkilata girişini deaktiv etmək?',
  'Отмена заказа': 'Sifarişin ləğvi',
  открыт: 'açıqdır',
  Открыт: 'Açıqdır',
  Открыта: 'Açıqdır',
  'Открытые заказы': 'Açıq sifarişlər',
  'Открытые смены': 'Açıq növbələr',
  Открыть: 'Aç',
  'Открыть боковое меню': 'Yan menyunu aç',
  'Открыть заказ': 'Sifarişi aç',
  'Открыть меню': 'Menyunu aç',
  'Остановить сессию': 'Sessiyanı dayandır',
  Отменен: 'Ləğv edilib',
  Отменено: 'Ləğv edilib',
  Отменить: 'Ləğv et',
  'Отменить заказ?': 'Sifarişi ləğv etmək?',
  'Очередь уведомлений': 'Bildiriş növbəsi',
  'Пароль должен содержать минимум 6 символов.': 'Şifrə ən azı 6 simvoldan ibarət olmalıdır.',
  Пауза: 'Fasilə',
  'Пауза сессии пока не поддержана сервером': 'Sessiya fasiləsi serverdə hələ dəstəklənmir',
  'Перевод на карту': 'Karta köçürmə',
  Переводы: 'Köçürmələr',
  'Перемещение +': 'Köçürmə +',
  'Перемещение -': 'Köçürmə -',
  'Период организации': 'Təşkilat dövrü',
  Периоды: 'Dövrlər',
  'Периоды на проверке': 'Yoxlamada olan dövrlər',
  План: 'Plan',
  'Платежей нет': 'Ödəniş yoxdur',
  'Платежи платформе': 'Platformaya ödənişlər',
  Платформа: 'Platforma',
  Позиция: 'Mövqe',
  'По товару пока нет складских движений.': 'Bu məhsul üzrə anbar hərəkəti hələ yoxdur.',
  'По фильтру мест нет': 'Filtrə uyğun yer yoxdur',
  Подтвержден: 'Təsdiqlənib',
  'Подтверждение платежей организаций по доле платформы.':
    'Təşkilatların platforma payı ödənişlərinin təsdiqi.',
  Подтверждён: 'Təsdiqlənib',
  Поиск: 'Axtarış',
  'Поиск товара или услуги': 'Məhsul və ya xidmət axtarışı',
  'Показать пароль': 'Şifrəni göstər',
  'Получатель / поставщик': 'Alan / təchizatçı',
  'Пользователей на организацию': 'Təşkilat üzrə istifadəçi sayı',
  Пользователи: 'İstifadəçilər',
  Пользователь: 'İstifadəçi',
  'Пользователь с таким email не найден. Сначала создайте его в Supabase Authentication.':
    'Bu email ilə istifadəçi tapılmadı. Əvvəlcə onu Supabase Authentication-da yaradın.',
  'Порог крупного расхода': 'Böyük xərc həddi',
  Порядок: 'Sıra',
  'Порядок не может быть отрицательным.': 'Sıra mənfi ola bilməz.',
  'После оплаты заказов или ручного ввода здесь появятся финансовые операции.':
    'Sifarişlər ödənildikdən və ya əl ilə daxil edildikdən sonra maliyyə əməliyyatları burada görünəcək.',
  Поставщик: 'Təchizatçı',
  Почасовая: 'Saatlıq',
  'Почасовой тариф': 'Saatlıq tarif',
  'При расхождении укажите комментарий.': 'Uyğunsuzluq varsa, şərh yazın.',
  'Принять оплату': 'Ödənişi qəbul et',
  Приостановлена: 'Dayandırılıb',
  'Причина force close': 'Məcburi bağlama səbəbi',
  'Причина отклонения': 'Rədd səbəbi',
  'Причина отмены документа': 'Sənədin ləğv səbəbi',
  Проведен: 'Keçirilib',
  'Проверить склад': 'Anbarı yoxla',
  'Проверка PIN': 'PIN yoxlaması',
  'Проверка периода и утверждение начисления доли платформы.':
    'Dövrün yoxlanması və platforma payı hesablamasının təsdiqi.',
  'Проверьте значения в форме.': 'Formadakı dəyərləri yoxlayın.',
  Продажа: 'Satış',
  'Продажная цена': 'Satış qiyməti',
  Просрочено: 'Gecikib',
  'Профиль пользователя пока не создан.': 'İstifadəçi profili hələ yaradılmayıb.',
  Процент: 'Faiz',
  'Публичный адрес': 'Publik ünvan',
  'Публичный адрес Vercel и формат ссылок организаций.':
    'Vercel publik ünvanı və təşkilat linklərinin formatı.',
  'Путь логотипа': 'Loqo yolu',
  'Рабочая панель': 'İş paneli',
  'Рабочее место': 'İş yeri',
  'Рабочее место сотрудника не найдено.': 'Əməkdaşın iş yeri tapılmadı.',
  'Раздел в подготовке': 'Bölmə hazırlanır',
  'Размер изображения не должен превышать 5 MB.': 'Şəklin ölçüsü 5 MB-dan çox olmamalıdır.',
  Расходы: 'Xərclər',
  'Региональные параметры': 'Regional parametrlər',
  'Регулярные расходы': 'Müntəzəm xərclər',
  'Регулярные расходы создают финансовые операции по расписанию.':
    'Müntəzəm xərclər cədvəl üzrə maliyyə əməliyyatları yaradır.',
  'Редактировать категорию': 'Kateqoriyanı redaktə et',
  'Редактировать место': 'Yeri redaktə et',
  'Редактировать организацию': 'Təşkilatı redaktə et',
  'Редактировать товар': 'Məhsulu redaktə et',
  'Редактировать услугу': 'Xidməti redaktə et',
  'Резерв заказа': 'Sifariş rezervi',
  'Резерв комбо': 'Kombo rezervi',
  'Роль не задана': 'Rol təyin edilməyib',
  Руль: 'Sükan',
  'Ряд Y': 'Y sırası',
  'Ряд должен быть больше 0.': 'Sıra 0-dan böyük olmalıdır.',
  Свободно: 'Boşdur',
  'Связанная запись не найдена или недоступна.': 'Əlaqəli qeyd tapılmadı və ya əlçatan deyil.',
  Сессии: 'Sessiyalar',
  'Сессии, PIN-код рабочего экрана и подтверждения администратора.':
    'Sessiyalar, iş ekranı PIN-kodu və administrator təsdiqləri.',
  'Сессия истекла. Войдите снова.': 'Sessiya bitib. Yenidən daxil olun.',
  'Сессия не начата': 'Sessiya başlamayıb',
  'Система и интеграции': 'Sistem və inteqrasiyalar',
  Склад: 'Anbar',
  'Склад пуст': 'Anbar boşdur',
  Скопировано: 'Kopyalandı',
  'Скрыть боковое меню': 'Yan menyunu gizlət',
  'Скрыть меню': 'Menyunu gizlət',
  'Скрыть пароль': 'Şifrəni gizlət',
  'Следующее создание': 'Növbəti yaradılma',
  Смена: 'Növbə',
  'Сменить PIN': 'PIN-i dəyiş',
  Смены: 'Növbələr',
  'Сначала завершите сессию, затем переводите заказ к оплате.':
    'Əvvəlcə sessiyanı tamamlayın, sonra sifarişi ödənişə keçirin.',
  'Сначала остановите сессию, затем переводите заказ к оплате.':
    'Əvvəlcə sessiyanı dayandırın, sonra sifarişi ödənişə keçirin.',
  'Сначала найдите существующего пользователя.': 'Əvvəlcə mövcud istifadəçini tapın.',
  'Сколько клиент оставил': 'Müştəri nə qədər qoydu',
  'Снятие резерва заказа': 'Sifariş rezervinin silinməsi',
  'Снятие резерва комбо': 'Kombo rezervinin silinməsi',
  'Собрать комбо': 'Kombo yığ',
  'Сейчас:': 'İndi:',
  'Создайте первое fixed-комбо из товаров и услуг.':
    'Məhsul və xidmətlərdən ilk sabit kombonu yaradın.',
  'Создайте первую категорию для товаров, услуг или мест.':
    'Məhsullar, xidmətlər və ya yerlər üçün ilk kateqoriyanı yaradın.',
  'Создайте первую организацию и назначьте администратора из существующих Auth-пользователей.':
    'İlk təşkilatı yaradın və mövcud Auth istifadəçilərindən administrator təyin edin.',
  'Создайте первую услугу.': 'İlk xidməti yaradın.',
  'Создайте первый товар или измените фильтры.': 'İlk məhsulu yaradın və ya filtrləri dəyişin.',
  'Создан хотя бы один активный сотрудник.': 'Ən azı bir aktiv əməkdaş yaradılıb.',
  'Созданы категории для доходов и расходов.': 'Gəlir və xərclər üçün kateqoriyalar yaradılıb.',
  'Созданы рабочие места организации.': 'Təşkilatın iş yerləri yaradılıb.',
  'Создать заказ': 'Sifariş yarat',
  'Создать категорию': 'Kateqoriya yarat',
  'Создать место': 'Yer yarat',
  'Создать организацию': 'Təşkilat yarat',
  'Создать товар': 'Məhsul yarat',
  'Создать услугу': 'Xidmət yarat',
  'Состав не заполнен.': 'Tərkib doldurulmayıb.',
  Сотрудник: 'Əməkdaş',
  'Сотрудник больше не сможет войти в рабочий интерфейс этой организации. История его действий и операций сохранится.':
    'Əməkdaş bu təşkilatın iş interfeysinə artıq daxil ola bilməyəcək. Onun əməliyyat tarixçəsi saxlanılacaq.',
  'Сотрудник не выбран.': 'Əməkdaş seçilməyib.',
  Сотрудники: 'Əməkdaşlar',
  'Сотрудники недоступны': 'Əməkdaşlar əlçatan deyil',
  'Сотрудники пока не добавлены': 'Əməkdaşlar hələ əlavə edilməyib',
  Списание: 'Silinmə',
  Средний: 'Orta',
  'Срок оплаты доли, дней': 'Pay ödənişi müddəti, gün',
  Старт: 'Start',
  Стоп: 'Stop',
  Стол: 'Masa',
  'Столы, VIP-кабинеты, игровые зоны и другие локации.':
    'Masalar, VIP kabinetlər, oyun zonaları və digər lokasiyalar.',
  'Страница зарезервирована под следующий финансовый отчёт.':
    'Səhifə növbəti maliyyə hesabatı üçün ayrılıb.',
  Сумма: 'Məbləğ',
  'Сумма заказа': 'Sifariş məbləği',
  'Таймаут сессии, минут': 'Sessiya timeout-u, dəqiqə',
  'Такая запись уже существует.': 'Belə qeyd artıq mövcuddur.',
  'Такое уведомление уже создано.': 'Belə bildiriş artıq yaradılıb.',
  'Тариф не может быть отрицательным.': 'Tarif mənfi ola bilməz.',
  Телефон: 'Telefon',
  'Товаров пока нет': 'Məhsullar hələ yoxdur',
  'Товаров со складским учетом пока нет.': 'Anbar uçotu olan məhsullar hələ yoxdur.',
  Товары: 'Məhsullar',
  'У пользователя уже есть активный доступ.': 'İstifadəçinin artıq aktiv girişi var.',
  'У сотрудника уже открыта смена.': 'Əməkdaşın artıq açıq növbəsi var.',
  Уведомления: 'Bildirişlər',
  'Удалить рабочее место': 'İş yerini sil',
  'Удалить цифру': 'Rəqəmi sil',
  'Удаление позиции': 'Mövqenin silinməsi',
  'Укажите UUID пользователя Auth.': 'Auth istifadəçisinin UUID-sini göstərin.',
  'Укажите UUID существующего Auth-пользователя.': 'Mövcud Auth istifadəçisinin UUID-sini göstərin.',
  'Укажите единицу.': 'Vahidi göstərin.',
  'Укажите пользовательский тип.': 'Fərdi növü göstərin.',
  'Укажите причину отмены заказа.': 'Sifarişin ləğv səbəbini göstərin.',
  'Укажите сумму, которую клиент реально оставил.':
    'Müştərinin real qoyduğu məbləği göstərin.',
  'Укажите фактическую сумму клиента в конце заказа.':
    'Sifarişin sonunda müştərinin faktiki məbləğini göstərin.',
  'Укажите фиксированную цену.': 'Sabit qiyməti göstərin.',
  'Укажите часовой пояс.': 'Saat qurşağını göstərin.',
  'Услуг пока нет': 'Xidmətlər hələ yoxdur',
  Услуги: 'Xidmətlər',
  'Учёт без цены': 'Qiymətsiz uçot',
  'Фактическая сумма': 'Faktiki məbləğ',
  'Фактическая сумма от клиента': 'Müştəridən faktiki məbləğ',
  'Фактическая наличность': 'Faktiki nağd pul',
  Фиксированная: 'Sabit',
  'Фиксированная цена': 'Sabit qiymət',
  'Фиксированные и почасовые услуги организации.':
    'Təşkilatın sabit və saatlıq xidmətləri.',
  'Финансовые категории': 'Maliyyə kateqoriyaları',
  'Финансовые настройки организации. Долю платформы меняет только владелец платформы.':
    'Təşkilatın maliyyə ayarları. Platforma payını yalnız platforma sahibi dəyişir.',
  'Текущие правила расчёта': 'Cari hesablama qaydaları',
  'Эти показатели показывают, как сейчас делится чистая прибыль и какой финансовый цикл используется для периодов.':
    'Bu göstəricilər xalis mənfəətin hazırda necə bölündüyünü və dövrlər üçün hansı maliyyə dövrünün istifadə edildiyini göstərir.',
  'Процент, который будет начислен Freedom Platform после утверждения финансового периода.':
    'Maliyyə dövrü təsdiqləndikdən sonra Freedom Platform üçün hesablanacaq faiz.',
  'Доля владельца': 'Sahibin payı',
  'Оставшаяся часть чистой прибыли после доли платформы.':
    'Platforma payından sonra qalan xalis mənfəət hissəsi.',
  'Текущий финансовый период': 'Cari maliyyə dövrü',
  'Текущий расчётный период': 'Cari hesablama dövrü',
  'Карточки дохода, COGS, прибыли, оплаты картой и cash out считаются внутри этого периода.':
    'Gəlir, COGS, mənfəət, kartla ödəniş və cash out kartları bu dövr daxilində hesablanır.',
  'Если день 15, текущий период идёт с 15-го числа до 14-го числа следующего месяца.':
    'Gün 15-dirsə, cari dövr ayın 15-dən növbəti ayın 14-nə qədər davam edir.',
  'Срок оплаты доли платформы': 'Platforma payının ödəniş müddəti',
  'После закрытия периода долю платформы нужно оплатить в течение этого количества дней.':
    'Dövr bağlandıqdan sonra platforma payı bu gün sayı ərzində ödənilməlidir.',
  дней: 'gün',
  'Следующее закрытие финансового месяца': 'Növbəti maliyyə ayı bağlanışı',
  'Проверьте, что все смены закрыты, расходы внесены, а спорные оплаты исправлены до отправки периода.':
    'Dövr göndərilməzdən əvvəl bütün növbələrin bağlandığını, xərclərin daxil edildiyini və mübahisəli ödənişlərin düzəldildiyini yoxlayın.',
  'Рабочие настройки организации': 'Təşkilatın iş ayarları',
  'Эти параметры влияют на отчёты, создание периодов и проверку крупных расходов.':
    'Bu parametrlər hesabatlara, dövrlərin yaradılmasına və böyük xərclərin yoxlanmasına təsir edir.',
  'Настройки владельца платформы': 'Platforma sahibinin ayarları',
  'Вы можете изменить долю платформы для этой организации.':
    'Bu təşkilat üçün platforma payını dəyişə bilərsiniz.',
  'Эти значения назначает только владелец платформы.':
    'Bu dəyərləri yalnız platforma sahibi təyin edir.',
  'Срок оплаты доли платформы, дней': 'Platforma payının ödəniş müddəti, gün',
  'Требовать подтверждение крупных расходов': 'Böyük xərclər üçün təsdiq tələb et',
  'для The Liga сейчас логично держать 15, потому что организация начала работу 15 августа.':
    'The Liga üçün hazırda 15 saxlamaq məntiqlidir, çünki təşkilat 15 avqustda işə başlayıb.',
  'если включено подтверждение, расходы от этой суммы будут попадать на проверку перед закрытием периода.':
    'təsdiq aktivdirsə, bu məbləğdən başlayan xərclər dövr bağlanmazdan əvvəl yoxlamaya düşəcək.',
  'используется только как валюта отображения финансовых отчётов.':
    'yalnız maliyyə hesabatlarının göstərilmə valyutası kimi istifadə olunur.',
  'Финансовые периоды': 'Maliyyə dövrləri',
  'Создать период': 'Dövr yarat',
  'Выберите даты, система пересчитает доходы, COGS, расходы, прибыль и отправит период на проверку.':
    'Tarixləri seçin, sistem gəlirləri, COGS, xərcləri, mənfəəti yenidən hesablayacaq və dövrü yoxlamaya göndərəcək.',
  'Список периодов': 'Dövrlər siyahısı',
  'Редактирование пересчитывает период. Удаление помечает период как удалённый, закрытые периоды остаются архивом.':
    'Redaktə dövrü yenidən hesablayır. Silmə dövrü silinmiş kimi işarələyir, bağlı dövrlər arxivdə qalır.',
  Удалённые: 'Silinənlər',
  'Периоды загружаются...': 'Dövrlər yüklənir...',
  Период: 'Dövr',
  Прибыль: 'Mənfəət',
  Доля: 'Pay',
  Владельцу: 'Sahibə',
  Удалён: 'Silinib',
  'Периодов в этом фильтре нет.': 'Bu filtrdə dövr yoxdur.',
  'Изменить период': 'Dövrü dəyiş',
  'После сохранения суммы будут пересчитаны по новым датам.':
    'Yadda saxladıqdan sonra məbləğlər yeni tarixlər üzrə yenidən hesablanacaq.',
  'Удалить период': 'Dövrü sil',
  'Период будет помечен как удалённый. Физически финансовые записи не удаляются.':
    'Dövr silinmiş kimi işarələnəcək. Maliyyə qeydləri fiziki olaraq silinmir.',
  'Например: неверные даты периода': 'Məsələn: dövr tarixləri səhvdir',
  'Финансовый период': 'Maliyyə dövrü',
  'Финансовый период закрыт и не может быть изменён.':
    'Maliyyə dövrü bağlanıb və dəyişdirilə bilməz.',
  'Финансовый раздел платформы.': 'Platformanın maliyyə bölməsi.',
  'Финансовый центр организации: доходы, расходы, закупки, cash flow, P&L и доля Freedom Platform.':
    'Təşkilatın maliyyə mərkəzi: gəlirlər, xərclər, satınalmalar, cash flow, P&L və Freedom Platform payı.',
  Финансы: 'Maliyyə',
  'Финансы выбранной организации и настройка процента доли Freedom Platform.':
    'Seçilmiş təşkilatın maliyyəsi və Freedom Platform pay faizinin ayarlanması.',
  'Финансы организации': 'Təşkilat maliyyəsi',
  'Финансы платформы': 'Platforma maliyyəsi',
  'Флаги интеграций и технического обслуживания.':
    'İnteqrasiyalar və texniki xidmət bayraqları.',
  Фото: 'Şəkil',
  'Фото не загружено': 'Şəkil yüklənməyib',
  'Фото отсутствует': 'Şəkil yoxdur',
  'Характеристики не заполнены.': 'Xüsusiyyətlər doldurulmayıb.',
  'Хранение audit logs, дней': 'Audit log saxlama müddəti, gün',
  'Хранилище на организацию, GB': 'Təşkilat üzrə saxlama yeri, GB',
  Цена: 'Qiymət',
  'Цена вручную': 'Qiymət əl ilə',
  'Цена будет указана вручную при оплате.':
    'Qiymət ödəniş zamanı əl ilə göstəriləcək.',
  'Цена комбо': 'Kombo qiyməti',
  'Цена не может быть отрицательной.': 'Qiymət mənfi ola bilməz.',
  'Часовой пояс': 'Saat qurşağı',
  Чаевые: 'Çaypulu',
  'Чаевые не могут быть отрицательными.': 'Çaypulu mənfi ola bilməz.',
  Частично: 'Qismən',
  'Частично оплачено': 'Qismən ödənilib',
  Черновик: 'Qaralama',
  'Чистая прибыль': 'Xalis mənfəət',
  'Шаблоны смен': 'Növbə şablonları',
  'Шаг должен быть больше 0.': 'Addım 0-dan böyük olmalıdır.',
  'Шаг расчета': 'Hesablama addımı',
  'Шаг тарификации': 'Tarif addımı',
  Ширина: 'En',
  'Ширина от 1 до 12.': 'En 1-dən 12-yə qədər olmalıdır.',
  Широкий: 'Geniş',
  'Этот заказ уже оплачен.': 'Bu sifariş artıq ödənilib.',
  'Это администратор организации.': 'Bu təşkilat administratorudur.',
  'Этот комментарий сохранится в заказе после оплаты или завершения.':
    'Bu şərh ödənişdən və ya tamamlamadan sonra sifarişdə saxlanılacaq.',
  'Chat ID хранится в настройках организации. Bot token должен быть только в Edge Function secret `TELEGRAM_BOT_TOKEN`.':
    'Chat ID təşkilat ayarlarında saxlanılır. Bot token yalnız Edge Function secret `TELEGRAM_BOT_TOKEN` daxilində olmalıdır.',
  'PIN блокировки сайта': 'Sayt kilidi PIN-i',
  'PNG, JPG или WebP до 5 MB. Файл будет сжат и сохранён в Storage.':
    '5 MB-a qədər PNG, JPG və ya WebP. Fayl sıxılacaq və Storage-də saxlanılacaq.',
  'Telegram уведомления': 'Telegram bildirişləri',
  'Администратор выбирается из уже созданных Supabase Auth пользователей.':
    'Administrator artıq yaradılmış Supabase Auth istifadəçilərindən seçilir.',
  'Администратор организации может добавить места и настроить их расположение.':
    'Təşkilat administratoru yerlər əlavə edə və onların yerləşməsini qura bilər.',
  Админка: 'Admin panel',
  'Активные доступы': 'Aktiv girişlər',
  'Активные доступы:': 'Aktiv girişlər:',
  'Базовые параметры Freedom Platform: ссылки организаций, безопасность, финансы, лимиты и системные интеграции.':
    'Freedom Platform baza parametrləri: təşkilat linkləri, təhlükəsizlik, maliyyə, limitlər və sistem inteqrasiyaları.',
  'Базовые параметры текущей организации. Редактирование будет расширено следующим этапом.':
    'Cari təşkilatın baza parametrləri. Redaktə növbəti mərhələdə genişləndiriləcək.',
  'Без категории': 'Kateqoriyasız',
  'Быстрые разделы': 'Sürətli bölmələr',
  'В платформу': 'Platformaya',
  'Введите PIN сотрудника, чтобы продолжить работу.':
    'İşə davam etmək üçün əməkdaş PIN-ini daxil edin.',
  'Вернуться в платформу': 'Platformaya qayıt',
  'Вести складской учет': 'Anbar uçotunu apar',
  'Включить Telegram': 'Telegram-ı aktiv et',
  'Включить таймер': 'Taymeri aktiv et',
  'Все документы': 'Bütün sənədlər',
  'Все финансы': 'Bütün maliyyə',
  'Вход в систему': 'Sistemə giriş',
  Выберите: 'Seçin',
  'Выберите товары или откройте текущий заказ для полной корзины.':
    'Tam səbət üçün məhsulları seçin və ya cari sifarişi açın.',
  Выгода: 'Fayda',
  Выйти: 'Çıx',
  'Выйти из аккаунта': 'Hesabdan çıx',
  Готовность: 'Hazırlıq',
  'Группы для товаров, услуг и мест. Категория необязательна для элементов каталога.':
    'Məhsullar, xidmətlər və yerlər üçün qruplar. Kataloq elementləri üçün kateqoriya vacib deyil.',
  'Дата добавления': 'Əlavə edilmə tarixi',
  Движение: 'Hərəkət',
  Действие: 'Əməliyyat',
  Действия: 'Əməliyyatlar',
  'Действий пока нет': 'Hələ əməliyyat yoxdur',
  Детали: 'Detallar',
  Длительность: 'Müddət',
  'Для продаж, сессий и оплат нужна открытая смена.':
    'Satışlar, sessiyalar və ödənişlər üçün açıq növbə lazımdır.',
  'Для этой зоны не включён таймер. Администратор может включить тариф в настройках места.':
    'Bu zona üçün taymer aktiv deyil. Administrator yer ayarlarında tarifi aktiv edə bilər.',
  Добавить: 'Əlavə et',
  'Добавить компонент': 'Komponent əlavə et',
  'Добавить позицию': 'Mövqe əlavə et',
  'Добавить правило': 'Qayda əlavə et',
  'Добавить рабочее место': 'İş yeri əlavə et',
  'Добавить справа': 'Sağa əlavə et',
  'Добавьте первое место и расположите его на сетке.':
    'İlk yeri əlavə edin və onu şəbəkədə yerləşdirin.',
  'Документов пока нет': 'Sənədlər hələ yoxdur',
  Долг: 'Borc',
  'Долг платформе': 'Platformaya borc',
  'Доступ не настроен': 'Giriş qurulmayıb',
  'Доступ открыт только для созданных пользователей платформы и организаций.':
    'Giriş yalnız yaradılmış platforma və təşkilat istifadəçiləri üçün açıqdır.',
  'Доступно только владельцу платформы. Действие необратимое.':
    'Yalnız platforma sahibinə əlçatandır. Əməliyyat geri qaytarılmır.',
  'Доступы и роли команды.': 'Komandanın girişləri və rolları.',
  Ежегодно: 'İllik',
  Ежеквартально: 'Rüblük',
  Ежемесячно: 'Aylıq',
  Еженедельно: 'Həftəlik',
  'Есть расхождение по наличности. Добавьте короткий комментарий перед закрытием.':
    'Nağd pulda uyğunsuzluq var. Bağlamadan əvvəl qısa şərh əlavə edin.',
  'Живые показатели организаций, финансов и рабочих процессов платформы.':
    'Təşkilatlar, maliyyə və platforma iş prosesləri üzrə canlı göstəricilər.',
  Журнал: 'Jurnal',
  'Журнал действий': 'Əməliyyat jurnalı',
  'Журнал действий недоступен': 'Əməliyyat jurnalı əlçatan deyil',
  'Журнал изменений заказа': 'Sifariş dəyişiklikləri jurnalı',
  'Здесь не записывается каждый клик. Логируются значимые переходы по админ-разделам и операции, которые меняют данные или деньги.':
    'Burada hər klik yazılmır. Yalnız admin bölmələri üzrə vacib keçidlər və məlumatları və ya pulu dəyişən əməliyyatlar qeyd olunur.',
  'Завершить заказ': 'Sifarişi tamamla',
  'Завершить сессию': 'Sessiyanı tamamla',
  'Завершить смену': 'Növbəni tamamla',
  Загрузка: 'Yüklənir',
  'Загрузка документов': 'Sənədlər yüklənir',
  'Загрузка заказа': 'Sifariş yüklənir',
  'Загрузка заказов': 'Sifarişlər yüklənir',
  'Загрузка запросов': 'Sorğular yüklənir',
  'Загрузка истории': 'Tarixçə yüklənir',
  'Загрузка журнала': 'Jurnal yüklənir',
  'Загрузка категорий': 'Kateqoriyalar yüklənir',
  'Загрузка комбо': 'Kombo yüklənir',
  'Загрузка мест': 'Yerlər yüklənir',
  'Загрузка организаций': 'Təşkilatlar yüklənir',
  'Загрузка позиций...': 'Mövqelər yüklənir...',
  'Загрузка показателей...': 'Göstəricilər yüklənir...',
  'Загрузка пользователей': 'İstifadəçilər yüklənir',
  'Загрузка рабочего места': 'İş yeri yüklənir',
  'Загрузка склада': 'Anbar yüklənir',
  'Загрузка смен': 'Növbələr yüklənir',
  'Загрузка смены': 'Növbə yüklənir',
  'Загрузка сотрудников': 'Əməkdaşlar yüklənir',
  'Загрузка товаров': 'Məhsullar yüklənir',
  'Загрузка услуг': 'Xidmətlər yüklənir',
  'Загрузка...': 'Yüklənir...',
  Задан: 'Təyin edilib',
  'Заказ без места': 'Yersiz sifariş',
  'Заказ на стол': 'Masa sifarişi',
  'Заказ не найден.': 'Sifariş tapılmadı.',
  'Заказов без места нет.': 'Yersiz sifariş yoxdur.',
  'Заказов по фильтру нет': 'Filtr üzrə sifariş yoxdur',
  'Заказы без места': 'Yersiz sifarişlər',
  'Закрыть сессию': 'Sessiyanı bağla',
  'Записать сумму': 'Məbləği yaz',
  'Запросить удаление': 'Silinmə sorğusu göndər',
  'Запросить удаление?': 'Silinmə sorğusu göndərmək?',
  'Запросить уточнение': 'Dəqiqləşdirmə istə',
  'Запросов по фильтру нет': 'Filtr üzrə sorğu yoxdur',
  'Запросы исправлений': 'Düzəliş sorğuları',
  Заявка: 'Sorğu',
  История: 'Tarixçə',
  'История продаж, неоплаченные заказы, отказы от оплаты и платежи.':
    'Satış tarixçəsi, ödənilməmiş sifarişlər, ödənişdən imtinalar və ödənişlər.',
  'Итоги дня по всем сменам без расчёта чистой прибыли.':
    'Xalis mənfəət hesablanmadan bütün növbələr üzrə gün yekunları.',
  'Итог будет введён вручную': 'Yekun əl ilə daxil ediləcək',
  Итого: 'Cəmi',
  'Итого к оплате': 'Ödəniləcək cəmi',
  Картой: 'Kartla',
  Tables: 'Masalar',
  Goods: 'Məhsullar',
  'Кассовая ответственность, закрытия, расхождения и передачи.':
    'Kassa məsuliyyəti, bağlamalar, uyğunsuzluqlar və təhvil vermələr.',
  'Код подтверждения неверный. Очистка не выполнена.':
    'Təsdiq kodu yanlışdır. Təmizləmə icra olunmadı.',
  'Каталог продаж с ценами и изображениями. Остатки меняются через складские документы.':
    'Qiymətlər və şəkillərlə satış kataloqu. Qalıqlar anbar sənədləri ilə dəyişir.',
  'Категории, товары, услуги и комбо.': 'Kateqoriyalar, məhsullar, xidmətlər və kombo.',
  Категория: 'Kateqoriya',
  Компоненты: 'Komponentlər',
  'Конструктор организации: места обслуживания, товары, услуги и категории.':
    'Təşkilat konstruktoru: xidmət yerləri, məhsullar, xidmətlər və kateqoriyalar.',
  Контроль: 'Nəzarət',
  'Код действия:': 'Əməliyyat kodu:',
  'Количество:': 'Miqdar:',
  'Корректировки:': 'Düzəlişlər:',
  'Коррекция времени сессии': 'Sessiya vaxtının düzəldilməsi',
  'Маршрут не существует или у вас нет прямой ссылки на нужный раздел.':
    'Marşrut mövcud deyil və ya lazımi bölməyə birbaşa linkiniz yoxdur.',
  Метод: 'Metod',
  'Метод оплаты': 'Ödəniş metodu',
  'Метод оплаты:': 'Ödəniş metodu:',
  'Мин.': 'Dəq.',
  Минимум: 'Minimum',
  'Минуты:': 'Dəqiqə:',
  'мобильная навигация': 'mobil naviqasiya',
  'Моя смена': 'Mənim növbəm',
  'На главную': 'Əsas səhifəyə',
  'Наборы из товаров и услуг с автоматической проверкой складской доступности.':
    'Anbar mövcudluğunun avtomatik yoxlanması ilə məhsul və xidmət dəstləri.',
  Назад: 'Geri',
  'Назначьте существующего Auth-пользователя администратором организации.':
    'Mövcud Auth istifadəçisini təşkilat administratoru təyin edin.',
  Наличными: 'Nağd',
  'Настроить места': 'Yerləri qur',
  'Настройка организации': 'Təşkilat qurulması',
  'Настройки платформы': 'Platforma ayarları',
  'Настройте одну, две или больше смен. Данные The Liga создаются вручную, не миграцией.':
    'Bir, iki və ya daha çox növbə qurun. The Liga məlumatları miqrasiya ilə yox, əl ilə yaradılır.',
  'Настройте названия смен без фиксированного времени. Итоги считаются по фактическому открытию и закрытию смены.':
    'Növbə adlarını sabit vaxt olmadan qurun. Yekunlar növbənin faktiki açılması və bağlanmasına görə hesablanır.',
  Начать: 'Başla',
  'Начать сессию': 'Sessiyanı başlat',
  'Не настроено окружение': 'Mühit qurulmayıb',
  'Неактивные организации:': 'Deaktiv təşkilatlar:',
  'Новый PIN': 'Yeni PIN',
  Обновление: 'Yenilənmə',
  Обычная: 'Adi',
  'Одобрите заявку, чтобы новый PIN начал работать.':
    'Yeni PIN-in işləməsi üçün sorğunu təsdiqləyin.',
  Одобрить: 'Təsdiqlə',
  'Ожидаемая касса': 'Gözlənilən kassa',
  Ожидается: 'Gözlənilir',
  Операция: 'Əməliyyat',
  Операции: 'Əməliyyatlar',
  Описание: 'Təsvir',
  Остаток: 'Qalıq',
  Отклонить: 'Rədd et',
  'Откройте эту страницу на компьютере или планшете в горизонтальном режиме.':
    'Bu səhifəni kompüterdə və ya planşeti üfüqi rejimdə açın.',
  'Открытые заказы и активные сессии уйдут в передачу следующей смене.':
    'Açıq sifarişlər və aktiv sessiyalar növbəti növbəyə təhvil veriləcək.',
  'Открыть смену': 'Növbəni aç',
  Отмена: 'İmtina',
  'Отменить заказ': 'Sifarişi ləğv et',
  Отправить: 'Göndər',
  Пароль: 'Şifrə',
  Передачи: 'Təhvil vermələr',
  'Перемещение мест и изменение размера скрыты на мобильной версии, чтобы не ломать рабочую сетку.':
    'İş şəbəkəsini pozmamaq üçün mobil versiyada yerlərin daşınması və ölçü dəyişməsi gizlədilib.',
  'Перетащите карточку мышкой на нужную клетку. Размер меняется кнопками W/H или через форму места.':
    'Kartı siçanla lazımi xanaya sürükləyin. Ölçü W/H düymələri və ya yer forması ilə dəyişir.',
  'Переход через полночь': 'Gecə yarısından keçid',
  'Перед удалением остаток товара должен быть 0.': 'Silməzdən əvvəl məhsul qalığı 0 olmalıdır.',
  'Пересчитано товаров': 'Yenidən hesablanan məhsullar',
  'Пересчитано товаров:': 'Yenidən hesablanan məhsullar:',
  'Периоды:': 'Dövrlər:',
  'Платежей нет.': 'Ödəniş yoxdur.',
  Платежи: 'Ödənişlər',
  'Платежи:': 'Ödənişlər:',
  Подтвердить: 'Təsdiqlə',
  Позиции: 'Mövqelər',
  'Позиции пока не добавлены.': 'Mövqelər hələ əlavə edilməyib.',
  'Позиции, сессия и корректировки.': 'Mövqelər, sessiya və düzəlişlər.',
  'Пользователи не назначены': 'İstifadəçilər təyin edilməyib',
  'Пользователи организации': 'Təşkilat istifadəçiləri',
  'Пользователь должен уже существовать в Supabase Authentication.':
    'İstifadəçi artıq Supabase Authentication-da mövcud olmalıdır.',
  'Понятная лента важных действий: кто заходил в ключевые разделы, создавал заказы, запускал сессии, принимал оплаты, закрывал смены и менял финансовые данные.':
    'Vacib əməliyyatların aydın lenti: kim əsas bölmələrə daxil olub, sifariş yaradıb, sessiya başladıb, ödəniş qəbul edib, növbəni bağlayıb və maliyyə məlumatlarını dəyişib.',
  После: 'Sonra',
  'Последние движения по товару. Показано до 100 записей.':
    'Məhsul üzrə son hərəkətlər. 100-ə qədər qeyd göstərilir.',
  'Последние документы': 'Son sənədlər',
  'Последние исправления': 'Son düzəlişlər',
  'Последние заказы': 'Son sifarişlər',
  'Последний журнал': 'Son jurnal',
  'Последние события': 'Son hadisələr',
  'Исправлений нет': 'Düzəliş yoxdur',
  Пример: 'Nümunə',
  'Пример:': 'Nümunə:',
  'Причина отмены': 'Ləğv səbəbi',
  'Причина корректировки': 'Düzəliş səbəbi',
  'Причина удаления': 'Silinmə səbəbi',
  'Причина удаления товара': 'Məhsulun silinmə səbəbi',
  'будет удалена из заказа сразу, а действие попадёт в журнал.':
    'sifarişdən dərhal silinəcək, əməliyyat isə jurnala düşəcək.',
  'Причина:': 'Səbəb:',
  'Например: пересчёт, ошибка ввода, поступление без документа.':
    'Məsələn: yenidən sayım, daxiletmə səhvi, sənədsiz daxilolma.',
  'Например: 50': 'Məsələn: 50',
  'Например: 5': 'Məsələn: 5',
  Провести: 'Keçir',
  'Провести сразу': 'Dərhal keçir',
  'Рабочая схема': 'İş sxemi',
  'Рабочая схема доступна только на компьютере': 'İş sxemi yalnız kompüterdə əlçatandır',
  'Рабочая схема пустая': 'İş sxemi boşdur',
  'Рабочее место только на компьютере': 'İş yeri yalnız kompüterdədir',
  'Рабочее состояние': 'İş vəziyyəti',
  'Рабочих мест пока нет': 'İş yerləri hələ yoxdur',
  'Раздел:': 'Bölmə:',
  Расход: 'Xərc',
  'Расходы на одобрение:': 'Təsdiq gözləyən xərclər:',
  Редактировать: 'Redaktə et',
  'Редактировать расход': 'Xərci redaktə et',
  'Редактировать пользователя': 'İstifadəçini redaktə et',
  'Режим выбора: fixed. Choice будет добавлен позже.':
    'Seçim rejimi: fixed. Choice sonra əlavə ediləcək.',
  'Режим открытия: цены скрыты, итог заказа вводится вручную.':
    'Açılış rejimi: qiymətlər gizlidir, sifariş yekunu əl ilə daxil edilir.',
  'Ручной итог': 'Əl ilə yekun',
  'Резервов нет.': 'Rezerv yoxdur.',
  Резервы: 'Rezervlər',
  Роль: 'Rol',
  Сверить: 'Yoxla',
  Сгенерировать: 'Yarat',
  'Сессии и резервы': 'Sessiyalar və rezervlər',
  'Сессий нет.': 'Sessiya yoxdur.',
  'Сессия и статус': 'Sessiya və status',
  Скидка: 'Endirim',
  'Складские документы': 'Anbar sənədləri',
  'Складской документ': 'Anbar sənədi',
  'Смена завершена. Рабочее место заблокировано до открытия новой смены.':
    'Növbə tamamlanıb. Yeni növbə açılana qədər iş yeri kilidlənib.',
  'Смена не найдена.': 'Növbə tapılmadı.',
  'Смена не открыта. Откройте смену, чтобы начать работу с заказами.':
    'Növbə açılmayıb. Sifarişlərlə işə başlamaq üçün növbəni açın.',
  'Событий пока нет.': 'Hadisə hələ yoxdur.',
  'Создайте `.env.local` в корне проекта и заполните публичные Supabase переменные. Значения ключей не выводятся в интерфейс.':
    'Layihə kökündə `.env.local` yaradın və publik Supabase dəyişənlərini doldurun. Açar dəyərləri interfeysdə göstərilmir.',
  Создана: 'Yaradılıb',
  'Создание мест работает. Для сохранения перемещения и размера примените миграцию `202607230010_workspace_layout_fields.sql`.':
    'Yer yaratmaq işləyir. Daşıma və ölçünü saxlamaq üçün `202607230010_workspace_layout_fields.sql` miqrasiyasını tətbiq edin.',
  'Создать документ': 'Sənəd yarat',
  'Создать комбо': 'Kombo yarat',
  'Сообщить об оплате': 'Ödəniş barədə bildir',
  'Состав заказа': 'Sifariş tərkibi',
  'Сотрудник создает запрос, администратор подтверждает или отклоняет действие.':
    'Əməkdaş sorğu yaradır, administrator əməliyyatı təsdiqləyir və ya rədd edir.',
  'Сотрудник меняет заказ сразу, а здесь администратор видит, что именно было изменено.':
    'Əməkdaş sifarişi dərhal dəyişir, administrator isə burada nəyin dəyişdiyini görür.',
  Сохранить: 'Saxla',
  'Сначала сохраните товар, затем измените остаток.':
    'Əvvəlcə məhsulu saxlayın, sonra qalığı dəyişin.',
  Средняя: 'Orta',
  Ставки: 'Dərəcələr',
  Статус: 'Status',
  Стоимость: 'Dəyər',
  'Столы, кабинеты, игровые зоны и другие места обслуживания.':
    'Masalar, kabinetlər, oyun zonaları və digər xidmət yerləri.',
  'Страница не найдена': 'Səhifə tapılmadı',
  'Сумма:': 'Məbləğ:',
  Таймер: 'Taymer',
  'Таймер считается локально; финальная сумма сессии пересчитывается сервером при завершении.':
    'Taymer lokal hesablanır; sessiyanın yekun məbləği tamamlananda serverdə yenidən hesablanır.',
  Тариф: 'Tarif',
  'Текущий остаток': 'Cari qalıq',
  Тип: 'Növ',
  'Тип:': 'Növ:',
  'Тип операции:': 'Əməliyyat növü:',
  'Тип цены': 'Qiymət növü',
  Товар: 'Məhsul',
  'Товары и услуги': 'Məhsullar və xidmətlər',
  'Товары, услуги, комбо': 'Məhsullar, xidmətlər, kombo',
  'Товар уже использовался в заказах, складе или комбо. Удаление невозможно, архивируйте товар.':
    'Məhsul artıq sifarişlərdə, anbarda və ya komboda istifadə olunub. Silmək mümkün deyil, məhsulu arxivləşdirin.',
  'Товар удален.': 'Məhsul silindi.',
  'Product has already been used and cannot be deleted. Archive it instead.':
    'Məhsul artıq istifadə olunub və silinə bilməz. Əvəzinə arxivləşdirin.',
  'Tip amount cannot be negative.': 'Çaypulu mənfi ola bilməz.',
  'Требовать approval крупных расходов': 'Böyük xərclər üçün təsdiq tələb et',
  'Требует внимания': 'Diqqət tələb edir',
  Убрать: 'Sil',
  Удалить: 'Sil',
  'Удалить позицию?': 'Mövqeni silmək?',
  'Удалено заказов': 'Silinən sifarişlər',
  'Удалено заказов:': 'Silinən sifarişlər:',
  'Удалено платежей': 'Silinən ödənişlər',
  'Удалено платежей:': 'Silinən ödənişlər:',
  'Удалено смен': 'Silinən növbələr',
  'Удалено смен:': 'Silinən növbələr:',
  'Удалить можно только товар без заказов, складских документов, движений, резервов и комбо. Если история уже есть, используйте архив.':
    'Yalnız sifarişləri, anbar sənədləri, hərəkətləri, rezervləri və kombosu olmayan məhsulu silmək olar. Tarixçə artıq varsa, arxivdən istifadə edin.',
  'Удалить товар': 'Məhsulu sil',
  'Удалить товар навсегда?': 'Məhsulu birdəfəlik silmək istəyirsiniz?',
  'Удаляет только тестовую операционную историю: заказы, платежи, смены, операционные дни, доходы от заказов и складские списания по заказам. Каталог, услуги, товары, складские документы и расходы остаются.':
    'Yalnız test əməliyyat tarixçəsini silir: sifarişlər, ödənişlər, növbələr, əməliyyat günləri, sifariş gəlirləri və sifariş üzrə anbar silinmələri. Kataloq, xidmətlər, məhsullar, anbar sənədləri və xərclər qalır.',
  'Увеличить на 1': '1 vahid artır',
  'Укажите причину изменения остатка.': 'Qalıq dəyişikliyinin səbəbini göstərin.',
  'Уменьшить на 1': '1 vahid azalt',
  'Управление tenant-организациями, статусами и первичными администраторами.':
    'Tenant təşkilatların, statusların və ilkin administratorların idarəsi.',
  'Управление сотрудниками и их доступом к организации.':
    'Əməkdaşların və onların təşkilata girişinin idarəsi.',
  Услуга: 'Xidmət',
  'Установить ставку': 'Dərəcəni təyin et',
  Фильтр: 'Filtr',
  'Финансовые данные пока пустые.': 'Maliyyə məlumatları hələ boşdur.',
  'Финансы организаций': 'Təşkilatların maliyyəsi',
  'Формат ссылки организации': 'Təşkilat linki formatı',
  Характеристики: 'Xüsusiyyətlər',
  'Цена за час': 'Saatlıq qiymət',
  'Цены, остатки и продажа.': 'Qiymətlər, qalıqlar və satış.',
  Частота: 'Tezlik',
  'Обновить остаток': 'Qalığı yenilə',
  'Остаток обновлен через складскую корректировку.':
    'Qalıq anbar düzəlişi ilə yeniləndi.',
  'Очистить тестовые заказы': 'Test sifarişlərini təmizlə',
  'Очистить тестовые заказы организации?': 'Təşkilatın test sifarişləri təmizlənsin?',
  'Очистка...': 'Təmizlənir...',
  'Очистка тестовых заказов': 'Test sifarişlərinin təmizlənməsi',
  'Ручная корректировка остатка': 'Qalığın əl ilə düzəlişi',
  'Черновики, проведенные и отмененные документы склада.':
    'Qaralamalar, keçirilmiş və ləğv edilmiş anbar sənədləri.',
  'Что уже контролируется системой': 'Sistem artıq nəyi yoxlayır',
  'Шаблон смены': 'Növbə şablonu',
  'Шаблонов пока нет.': 'Şablonlar hələ yoxdur.',
  Шаг: 'Addım',
  'Экран заблокирован': 'Ekran kilidlənib',
  'Эти параметры связаны с текущей архитектурой и видны владельцу платформы для контроля.':
    'Bu parametrlər cari arxitektura ilə bağlıdır və nəzarət üçün platforma sahibinə görünür.',
  'Язык': 'Dil',
  'Язык по умолчанию': 'Standart dil',
  'Язык системы': 'Sistem dili',
  'Язык интерфейса хранится локально на этом устройстве и применяется для администратора и рабочего места сотрудника в этом браузере.':
    'İnterfeys dili bu cihazda lokal saxlanılır və bu brauzerdə administratora və əməkdaşın iş yerinə tətbiq olunur.',
  'Язык, часовой пояс и региональные значения для новых организаций.':
    'Yeni təşkilatlar üçün dil, saat qurşağı və regional dəyərlər.',
  'Cash in': 'Pul girişi',
  'Cash out': 'Pul çıxışı',
  PlayStation: 'PlayStation',
  Столы: 'Masalar',
  'Оплата картой': 'Kartla ödəniş',
  'Итого владельцу': 'Sahibə qalan məbləğ',
  'Финансовая аналитика': 'Maliyyə analitikası',
  'Час пик': 'Pik saat',
  'Самая частая минута': 'Ən çox rast gəlinən dəqiqə',
  оплат: 'ödəniş',
  'Загрузка аналитики...': 'Analitika yüklənir...',
  'Выручка по направлениям (итого)': 'İstiqamətlər üzrə gəlir (cəmi)',
  'Выручка по направлениям (сегодня)': 'İstiqamətlər üzrə gəlir (bugün)',
  'Выручка по направлениям (смена)': 'İstiqamətlər üzrə gəlir (növbə)',
  'Выручка сегодня (всего)': 'Bugünkü gəlir (cəmi)',
  'Выручка смены (всего)': 'Növbə gəliri (cəmi)',
  'По карте': 'Kartla',
  'Операционный день': 'Əməliyyat günü',
  'Смена не открыта': 'Növbə açılmayıb',
  'Выручка смены': 'Növbə gəliri',
  'С момента': 'Başladığı vaxt',
  'Без учёта склада': 'Anbar uçotu yoxdur',
  таймер: 'taymer',
  'Финансовый центр организации: доходы, расходы, P&L, движение денег и аналитика оплат.':
    'Təşkilatın maliyyə mərkəzi: gəlirlər, xərclər, P&L, pul hərəkəti və ödəniş analitikası.',
  'Как считается:': 'Necə hesablanır:',
  'Все оплаченные и частично оплаченные доходы за текущий период по дате начисления: доходы из заказов и ручные доходы.':
    'Cari dövr üçün hesablama tarixinə görə bütün ödənilmiş və qismən ödənilmiş gəlirlər: sifariş gəlirləri və əl ilə daxil edilən gəlirlər.',
  'Себестоимость проданного за текущий период: сумма snapshot-себестоимости товаров и компонентов комбо в оплаченных заказах.':
    'Cari dövr üçün satılanların maya dəyəri: ödənilmiş sifarişlərdə məhsulların və kombo komponentlərinin snapshot maya dəyərinin cəmi.',
  'Доход минус COGS. Показывает прибыль после себестоимости проданного, до операционных расходов.':
    'Gəlir çıxılsın COGS. Satılanların maya dəyərindən sonra, əməliyyat xərclərindən əvvəl mənfəəti göstərir.',
  'Расходы, которые влияют на прибыль: не отменённые, не ожидающие подтверждения и не отклонённые, по дате начисления.':
    'Mənfəətə təsir edən xərclər: ləğv edilməmiş, təsdiq gözləməyən və rədd edilməmiş xərclər, hesablama tarixinə görə.',
  'Валовая прибыль минус операционные расходы. Это прибыль до расчёта доли платформы.':
    'Ümumi mənfəətdən əməliyyat xərcləri çıxılır. Bu, platforma payı hesablanmazdan əvvəlki mənfəətdir.',
  'Итоговая сумма для владельца после расчётов периода. Если чистая прибыль отрицательная, сумма тоже может быть отрицательной.':
    'Dövr hesablamalarından sonra sahibə qalan yekun məbləğ. Xalis mənfəət mənfidirsə, bu məbləğ də mənfi ola bilər.',
  'Фактически полученные деньги за текущий период по дате оплаты: оплаченные и частично оплаченные доходы.':
    'Cari dövr üçün ödəniş tarixinə görə faktiki daxil olan pul: ödənilmiş və qismən ödənilmiş gəlirlər.',
  'Фактически потраченные деньги за текущий период по дате оплаты: оплаченные расходы, закупки и платежи платформе.':
    'Cari dövr üçün ödəniş tarixinə görə faktiki çıxan pul: ödənilmiş xərclər, satınalmalar və platforma ödənişləri.',
  'Сумма завершённых платежей по карте за текущий период. Считается напрямую из платежей, чтобы видеть безналичный оборот.':
    'Cari dövr üçün kartla tamamlanmış ödənişlərin məbləği. Nağdsız dövriyyəni görmək üçün birbaşa ödənişlərdən hesablanır.',
  'Выручка по заказам PlayStation без товарных позиций. Товары из этих заказов считаются отдельно в карточке Товары.':
    'PlayStation sifarişləri üzrə gəlir, məhsul mövqeləri çıxılmaqla. Bu sifarişlərdəki məhsullar ayrıca Məhsullar kartında hesablanır.',
  'Выручка по заказам бильярда без товарных позиций. Товары из этих заказов считаются отдельно в карточке Товары.':
    'Bilyard sifarişləri üzrə gəlir, məhsul mövqeləri çıxılmaqla. Bu sifarişlərdəki məhsullar ayrıca Məhsullar kartında hesablanır.',
  'Выручка по столам и VIP-комнатам без товарных позиций, чтобы не смешивать посадку и продажи товаров.':
    'Masalar və VIP otaqlar üzrə gəlir, məhsul mövqeləri çıxılmaqla; oturma gəliri ilə məhsul satışları qarışmasın.',
  'Сумма товарных позиций во всех оплаченных заказах. Считается отдельно от площадок, чтобы не было двойного учёта.':
    'Bütün ödənilmiş sifarişlərdə məhsul mövqelərinin cəmi. İkiqat uçot olmaması üçün yerlərdən ayrıca hesablanır.',
  'Прибыль товаров': 'Məhsul mənfəəti',
  'Вся оплаченная выручка заказов со столов и VIP-комнат: услуги, товары, комбо и ручные позиции внутри этих заказов.':
    'Masalar və VIP otaqlardakı sifarişlərin bütün ödənilmiş gəliri: həmin sifarişlərdəki xidmətlər, məhsullar, kombo və əl ilə əlavə edilən mövqelər.',
  'Чистая прибыль по товарным позициям: сумма продаж товаров минус snapshot-себестоимость этих товаров в заказах.':
    'Məhsul mövqeləri üzrə xalis mənfəət: məhsul satışlarının cəmi çıxılsın sifarişlərdə saxlanmış snapshot maya dəyəri.',
  'Оплаченная выручка, которая не относится к PlayStation, бильярду, столам или товарам.':
    'PlayStation, bilyard, masalar və ya məhsullara aid olmayan ödənilmiş gəlir.',
  'Все завершённые платежи до сегодняшнего дня: распределение по 24 часам и шкала трафика от 1 до 10.':
    'Bu günə qədər bütün tamamlanmış ödənişlər: 24 saat üzrə bölgü və 1-dən 10-a qədər trafik şkalası.',
  Время: 'Vaxt',
  Проверка: 'Yoxlama',
  Проверил: 'Yoxladı',
  'Не проверено': 'Yoxlanmayıb',
  'Позиция не указана': 'Mövqe göstərilməyib',
  'Позиции не указаны': 'Mövqelər göstərilməyib',
  'Место / клиент': 'Yer / müştəri',
  Состав: 'Tərkib',
  Позиций: 'Mövqe sayı',
  'Открыт в': 'Açılış vaxtı',
  'Закрыт в': 'Bağlanış vaxtı',
  Источник: 'Mənbə',
  Кто: 'Kim',
  Объект: 'Obyekt',
  Код: 'Kod',
  'записал чаевые': 'çaypulunu qeyd etdi',
  'Чаевые:': 'Çaypulu:',
  'Сумма заказа:': 'Sifariş məbləği:',
  Системно: 'Sistem',
  Причина: 'Səbəb',
  Текущее: 'Cari',
  'Автоматически применено действием сотрудника': 'Əməkdaşın əməliyyatı ilə avtomatik tətbiq edildi',
  Выполнено: 'İcra olunub',
  авто: 'avto',
  'без SKU': 'SKU yoxdur',
  'без reference': 'reference yoxdur',
  задан: 'təyin edilib',
  'не задан': 'təyin edilməyib',
  'ожидает одобрения': 'təsdiq gözləyir',
  'добавил позицию в заказ': 'sifarişə mövqe əlavə etdi',
  'закрыл операционный день': 'əməliyyat gününü bağladı',
  'закрыл смену': 'növbəni bağladı',
  'запустил сессию': 'sessiyanı başlatdı',
  'зафиксировал излишек': 'artıqlığı qeydə aldı',
  'зафиксировал недостачу': 'çatışmazlığı qeydə aldı',
  'завершил сессию': 'sessiyanı tamamladı',
  'изменил заказ': 'sifarişi dəyişdi',
  'запросил уточнение по периоду': 'dövr üzrə dəqiqləşdirmə istədi',
  'создал заказ': 'sifariş yaratdı',
  'создал передачу смены': 'növbə təhvilini yaratdı',
  'создал расход': 'xərc yaratdı',
  'создал ручной доход': 'əl ilə gəlir yaratdı',
  'создал запрос на исправление': 'düzəliş sorğusu yaratdı',
  'изменил расход': 'xərci dəyişdi',
  'изменил финансовый период': 'maliyyə dövrünü dəyişdi',
  'удалил расход': 'xərci sildi',
  'удалил финансовый период': 'maliyyə dövrünü sildi',
  'удалил товар': 'məhsulu sildi',
  'очистил тестовые заказы': 'test sifarişlərini təmizlədi',
  товар: 'məhsul',
  организация: 'təşkilat',
  'синхронизировал доход по заказу': 'sifariş gəlirini sinxronlaşdırdı',
  'синхронизировал закупку': 'satınalmanı sinxronlaşdırdı',
  'сообщил об оплате доли платформы': 'platforma payı ödənişi barədə bildirdi',
  'одобрил исправление': 'düzəlişi təsdiqlədi',
  'одобрил расход': 'xərci təsdiqlədi',
  'одобрил финансовый период': 'maliyyə dövrünü təsdiqlədi',
  'отклонил исправление': 'düzəlişi rədd etdi',
  'отклонил оплату доли платформы': 'platforma payı ödənişini rədd etdi',
  'отклонил расход': 'xərci rədd etdi',
  'отклонил финансовый период': 'maliyyə dövrünü rədd etdi',
  'открыл раздел': 'bölməni açdı',
  'открыл смену': 'növbəni açdı',
  'переместил заказ': 'sifarişi köçürdü',
  'подтвердил оплату доли платформы': 'platforma payı ödənişini təsdiqlədi',
  'принудительно закрыл смену': 'növbəni məcburi bağladı',
  'принял оплату': 'ödənişi qəbul etdi',
  'установил ставку доли платформы': 'platforma payı dərəcəsini təyin etdi',
  'оформил отказ от оплаты': 'ödənişdən imtinanı rəsmiləşdirdi',
  'отправил финансовый период на проверку': 'maliyyə dövrünü yoxlamaya göndərdi',
  'заказ': 'sifariş',
  'позиция заказа': 'sifariş mövqeyi',
  'запрос исправления': 'düzəliş sorğusu',
  'сессия': 'sessiya',
  'смена': 'növbə',
  'передача смены': 'növbə təhvili',
  'операционный день': 'əməliyyat günü',
  'финансовая операция': 'maliyyə əməliyyatı',
  'финансовый период': 'maliyyə dövrü',
  'ставка доли платформы': 'platforma payı dərəcəsi',
  'платёж доли платформы': 'platforma payı ödənişi',
  'страница админки': 'admin səhifəsi',
  'поз.': 'mövqe',
  сейчас: 'indi',
  шаг: 'addım',
  'шт.': 'əd.',
}

export const translationEntries = Object.entries(azTranslations).sort(
  ([left], [right]) => right.length - left.length,
)
