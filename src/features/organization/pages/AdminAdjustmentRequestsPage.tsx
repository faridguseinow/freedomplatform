...
export function AdminAdjustmentRequestsPage() {
  ...
  const review = (requestId: string, decision: 'approved' | 'rejected') => {
    const comment = window.prompt(decision === 'approved' ? 'Комментарий к одобрению' : 'Причина отклонения')
  ...
  return (
    <section className="grid gap-5">
      <header className="grid gap-2">
        <h2 className="text-2xl font-semibold text-slate-950 sm:text-3xl">Запросы исправлений</h2>
      ...
      {requestsQuery.isLoading ? (
        <div className="inline-flex min-h-28 items-center justify-center gap-3 rounded-lg border border-slate-200 bg-white text-sm font-medium text-slate-600">
          <Loader2 className="size-4 animate-spin text-emerald-700" /> Загрузка запросов
        </div>
      ) : null}
          ...
          <article className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[1fr_auto]" key={request.id}>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-md bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">{statusLabel[request.status]}</span>
              </div>
              <p className="mt-2 text-sm text-slate-600">{request.reason}</p>
              <p className="mt-1 text-xs text-slate-500">
                Заказ: {request.order_id} · количество: {request.requested_quantity ?? '-'}
              </p>
            </div>
