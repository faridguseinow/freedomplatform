alter type public.financial_period_status add value if not exists 'cancelled';
...
begin
  ...
  if not public.is_organization_admin(period_row.organization_id) then
    raise exception 'Only organization admins can update financial periods.';
  end if;
...
begin
  ...
  if not public.is_organization_admin(period_row.organization_id) then
    raise exception 'Only organization admins can cancel financial periods.';
  end if;
